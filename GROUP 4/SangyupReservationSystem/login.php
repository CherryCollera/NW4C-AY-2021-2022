...
<?php 
  ...
  $verifySQL = "SELECT * FROM restaurant_info WHERE email = '$email' And password = '$password' And approve_status = 1;";

  $emailResult = $con->query($emailSQL);

  $passwordResult = $con->query($passwordSQL);

  $verify = $con->query($verifySQL);
  ...
  if ($emailResult->num_rows <= 0) {
    ...
    echo '<script>alert("This account is not verified.")</script>';
    echo '<script>window.location="login.php"</script>';
  }else{
    ...
    if ($_SESSION['role'] == 1) {
      ...
      echo '<script>window.location="admin/customers.php"</script>';
    } 
    // if ($_SESSION['role'] == 1) {
    //    echo '<script>window.location="dashboard/index.php"</script>';
    // }elseif ($_SESSION['role'] == 2) {
